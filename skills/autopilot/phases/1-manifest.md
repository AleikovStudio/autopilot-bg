...
| Generic | ≥32 chars of hex or base64 sitting next to `key`, `token`, `secret`, `password`, `ключ`, `токен`, `парола`, `достъп` |
...
3. Tell the user immediately, in one line, in plain language: „Изпрати ключ от Stripe — не го записах. Въведи го сам в `.env`, а този анулирай и си издай нов: вече мина през чата.“
...
The redacted brief, **word for word**, into `.autopilot/<slug>/<YYYY-MM-DD>-brief.md` — today's date, then `-brief.md`: `2026-08-07-brief.md`. The date is part of the name because a slug directory outlives one sitting; a month later „довърши и това“ arrives, and the files have to say which brief came when. Record the chosen name in `state.json` as `briefFile`, so nothing downstream has to guess it.

```markdown
# Първоначална задача

> Записано на <дата>. Този файл не се редактира — той е еталонът, с който
> се сверява готовият резултат.

<целият текст на потребителя, дословно, след заличаване на секретите>
```
...
- **Everything counts as brief** — the idea, the asides, the constraints, the „и още — добре би било…“, the stack preference, the deadline mentioned in passing.
- **Never edited again.** Later thoughts from the same sitting are appended under `## Допълнения`, dated, never merged into the original text. A brief dictated on a **later day** is a new file with that day's date — appending it to an older one erases the fact that the project was asked for twice.
...
```markdown
# Манифест на изискванията

Източник: `<дата>-brief.md`. Ред от този списък може да махне **само потребителят**.

| ID | От брифа (дословно) | Статус | Основание | Къде |
|----|---------------------|--------|-----------|------|
| R01 | „приема заявки за ремонт на техника“ | in-ticket | — | spec §2 → T02 |
| R02 | „ги записва в Google Таблица“ | in-ticket | — | spec §4 → T05 |
| R03 | „за да вижда клиентът статуса“ | in-spec | — | spec §6 |
| R04 | „и да дублира по SMS“ | dropped | потребител: „SMS не е нужно, само телеграм“ | — |
| R05 | „фирмени цветове на студиото“ | placeholder | цветовете не са предоставени | отчет |
| R06i | *(подразбира се)* някой трябва да чете заявките | deferred | Извън обхвата §9: в брифа нямаше администраторски панел | отчет |
```
...
- **`deferred` is not `dropped`.** Postponing is yours to decide; cancelling is not. Every `deferred` row appears in the final report under „какво не влезе“, so the user learns about it while the project is still fresh.
...
Mark with a trailing `i` (`R06i`) anything the brief clearly *implies* but never says — „приема заявки“ implies somewhere to read them; „онлайн магазин“ implies a way to pay.
...
- „телеграм бот, който приема заявки и ги записва в таблица“ → **two** requirements. One can work while the other doesn't.
- „красив съвременен дизайн“ → **one**, and it is inherently untestable — mark it and let the briefing turn it into something checkable, or let it stay a stated matter of taste.
...
„Разбих задачата на 23 изисквания — държа ги под контрол до самия край.“ No table in the chat. The file is the artifact; the chat is a mention of it.
