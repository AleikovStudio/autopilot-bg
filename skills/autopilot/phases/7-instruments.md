...
So the pane shows the numbers as of the moment it was opened, with the clocks running on top of them. **That is not a lie the user can walk into**: the footer says „обновено преди N“ about exactly the data on screen, and turns warning-coloured after five silent minutes.
...
- **Refresh before you answer.** „Как върви“, „покажи прогреса“ — re-point first, then speak, so the screen and your sentence agree.
...
open .autopilot/dashboard.html 2>/dev/null \
  ...
  || echo "отвори ръчно: .autopilot/dashboard.html"
```
...
```json
{
  ...
  "title": "Телеграм бот за заявки за ремонт",
  "mode": "semi",
  "depth": "normal",
  "tier": "T2",
  ...
  "tests": { "passed": 34, "failed": 0 },
  "debt": {
    "placeholders": ["R05 — фирмени цветове", "R11 — текстове на писма"],
    "assumptions": ["SQLite вместо Postgres — не е нужен сървър"],
    "emptyEnv": ["TELEGRAM_BOT_TOKEN", "GOOGLE_SHEETS_ID"]
  },
  "additions": ["Номер на заявката в потвърждението — заради R01"],
  "blind": null
}
```
...
## Stages — the answer to „къде сме сега“
...
## What „ход на разработката“ needs to be honest
...
Two consequences worth knowing: it is not the same number as „покритие на брифа“ — one measures the road travelled, the other measures value delivered, and they diverge on purpose — and it can only move forward, which is why a stalled bar means a stage that was never marked, not a build that is stuck.
...
- **`updatedAt` moves on every write.** The dashboard shows „обновено преди N“ from it and marks it in warning colour after five silent minutes — that is the user's only way to tell „работата тече“ from „агентът умря“.
...
2. In `dashboard.html`, replace everything between `/*autopilot:state:start*/` and `/*autopilot:state:end*/` with `const STATE = ` + the new JSON + `;`, one minified line — exactly as in Phase 0.
   ...
   ```bash
   grep -c 'autopilot:state:start' .autopilot/dashboard.html   # точно 1
   ```
   ...
   A dashboard that opens to a panel saying „таблото не успя да прочете състоянието“ is telling you exactly this, and it is not a lost run: the state is intact, only its copy inside the page is not.
...
|---|---|
| **Прогрес на проекта, %** | the one number for „колко остава до края“ — stages by weight, the build by finished tickets. Answers the question a ticket count cannot: how far along is the *project* |
| **Покритие на брифа, %** | *The* completion number. Ticket progress measures effort; brief coverage measures value. They diverge, and when they do, this one is right |
| **Етап сега, N от 8** | where the run is in the cycle, and how long it has been there. The first thing a person looks for and the last thing a ticket count answers |
...
With fewer than two finished tickets there is no median — the dashboard says „рано е за оценка“ rather than guessing.

## The one-line report to the user

After each ticket, in the chat: what became possible, and the count.

> Ботът приема заявки — 3 от 8 готови.
