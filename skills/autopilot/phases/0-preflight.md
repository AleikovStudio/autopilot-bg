...
The brief carries **the date it was dictated in its filename** — `2026-08-07-brief.md`. A slug directory outlives one conversation: the user comes back a month later with „довърши го“, a second brief gets appended, and a file called `brief.md` gives no way to tell which sitting is which. The date is the cheapest possible answer, and it sorts.
...
```markdown
# Как да четеш тази папка

- `dashboard.html` — отваря се сам в началото; може и с двоен клик.
  Етапи, прогрес, време, какво остава. Обновява се сам, докато тече сглобяването.
- `<проект>/<дата>-brief.md` — първоначалната ти задача, дума по дума. Не се редактира.
- `<проект>/manifest.md` — списък с изискванията и какво е станало с всяко от тях.
- `<проект>/spec.md` — спецификацията.
- `<проект>/tickets/` — тасковете, на които е разделена работата (при малък проект ги няма).

Ако сглобяването прекъсне — кажи на агента „продължи автопилота“ и той ще вдигне състоянието оттук.
```
...
2. Tell the user in one line where things stand: „Продължавам: 7 от 12 таска са готови, следващият е количката“.
