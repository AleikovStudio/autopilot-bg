...
Autopilot е рамка за разработка — умение (skill), което взима идеята ти, задава въпроси точно там, където в нея има развилки, а нататък само пише спецификацията, обмисля това, за което ти не си се сетил, разделя работата на таскове и сглобява проекта изцяло. Не ти трябва да четеш спецификации, да оценяваш таскове или да разбираш от код.
...
Отвори своя AI агент — Claude Code, Cursor, Codex — и му поднеси този текст:
...
**Windows**
```bash
git clone https://github.com/AleikovStudio/autopilot-bg.git "$env:TEMP\ap-bg"; Copy-Item "$env:TEMP\ap-bg\skills\autopilot" "$env:USERPROFILE\.claude\skills\autopilot" -Recurse -Force
```

**macOS / Linux**
```bash
git clone https://github.com/AleikovStudio/autopilot-bg.git /tmp/ap-bg && cp -r /tmp/ap-bg/skills/autopilot ~/.claude/skills/autopilot
```

Или натисни зеленото **Code → Download ZIP**, разархивирай и копирай папката `skills/autopilot` в `~/.claude/skills/`.
</details>
...
- **накрая — сляпо приемане:** отделен агент получава само твоя първоначален текст и готовия проект, спецификацията не му се дава. Той проверява резултата срещу **твоите думи**, а не срещу преразказ. Ако той и тракерът се разминат — ще го видиш в отчета.
