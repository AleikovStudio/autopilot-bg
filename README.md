...
| **Ръчен** | `/autopilot manual ...` или „ръчен режим“ | Въпроси, после твоето „ок“ на спецификацията и на списъка с подзадачи |
...
Обичайният проблем при сглобяване „по описание“: продиктувал си голям бриф, после тръгват въпроси, спецификация, подзадачи, код — а накрая половината от това, което си искал, просто го няма. Не защото са ти отказали, а защото на някоя стъпка е спряло да се споменава.
...
- **след разделянето** — всяко изискване има подзадача, и всяка подзадача има изискване (това хваща работа, която никой не е поръчвал);
...
**Етапите на целия цикъл са пред очите ти.** Осем стъпки от подготовка до приемане: подготовка, изисквания, брифинг, спецификация, план, разработка, код-ревю, приемане. Вижда се кои са минати, кой върви сега, кои са пропуснати и защо („ниво T0 — без разбивка на подзадачи“, „пълен автомат — самобрифинг“) и кои още предстоят.

**Времето върви в реално време.** Общ часовник на сглобяването, часовник на текущия етап и часовник на текущата подзадача тиктакат посекундно; при минатите етапи и готовите подзадачи се вижда колко време са отнели. Когато сглобяването приключи, часовниците спират на крайните цифри.

**Ход на разработката — какво е готово и какво се прави точно сега.** Щом планът е разделен, всички подзадачи се виждат наведнъж: колко са готови, колко са в процес, колко чакат реда си. Вървящите подзадачи са назовани по име, с тиктакащ часовник до всяка; готовите — с времето, което са отнели. Подзадачите, които не зависят една от друга, се сглобяват паралелно, и точно тук се вижда колко от тях летят едновременно.

**Лентата отгоре** е прогресът на целия проект, от нула до готов резултат. Тя отчита и минатите етапи, и дела готови подзадачи вътре в разработката, затова пълзи с всяка подзадача, а не стои половин ден на едно място.

| Показател | Какво означава |
|---|---|
| **Прогрес на проекта** | Една цифра „колко остава до края“ — по етапи и подзадачи заедно |
| **Покритие на брифа** | Колко от твоите изисквания са наистина затворени. Главната цифра: подзадачите може да са готови на 100%, а брифът — на 70% |
| **Текущ етап** | Къде е сглобяването в цикъла и откога върви този етап |
| **Изминало / остава** | Общо време и оценка по критичния път — по най-дългата верига зависими подзадачи, не по сбора на останалите |
| **Подзадачи** | Колко са направени от колко и колко са точно сега в процес. При малки проекти — „без разбивка“: подзадачи там няма по замисъл |
| **Дълг** | Заместители, решения, взети вместо теб, незапълнени променливи. Това е, което отделя „работи“ от „работи при теб“ |
| **Тестове** | Колко са минали след всяка подзадача — регресията се вижда веднага, а не накрая |
| **Повторения и време на подзадача** | Вижда се къде сглобяването е буксувало |

Дори когато проектът е малък и няма смисъл да се дели на подзадачи, таблото не е празно: етапите, изискванията, тестовете, комитът и заместителите се попълват по същия начин. Ако сесията прекъсне — кажи „продължи автопилота“ и агентът ще вдигне състоянието от файловете и ще продължи оттам, докъдето е стигнал.
...
| **План** | Спецификацията се реже на подзадачи — или не се реже, ако задачата е малка — и се разпределя на вълни: какво може да се сглобява едновременно и какво само по ред |
| **Разработка** | Една подзадача = един отделен агент с чиста памет, един комит; независимите подзадачи вървят паралелно |
| **Код-ревю** | След всяка подзадача: съответствие с брифа, съответствие със спецификацията, качество на кода |
...
### За разделянето на подзадачи

Всяка подзадача е отделен агент, който наново влиза в проекта: чете контрактите, изучава кода, разбира се със стека. Това струва скъпо. Затова Autopilot реже по нива, а не по принципа „колкото по-дребно, толкова по-надеждно“:

| Задача | Колко подзадачи |
|---|---|
...
|---|---|
| Всяка подзадача се изпълнява от отделен агент | Не се обърква в натрупания контекст и не чупи това, което е работело |
...
| Независимите подзадачи вървят паралелно, по няколко агента наведнъж | Да чакаш по ред неща, които не зависят едно от друго, са часове на празно |
| Един комит на подзадача | Твоите точки за връщане назад |
| Подзадачите с общи файлове вървят по ред | Иначе двама агенти ще презапишат работата си |
| Пълно пускане на тестовете след всяка подзадача | Регресията струва минути, а не цяла вечер |
| Паднала подзадача — повторение, после опит по друг начин | Ако и той не мине, агентът спира и обяснява по човешки |
...
- искаш да одобриш спецификацията и списъка с подзадачи, но не да се занимаваш с процеса — това е ръчният режим.
...
- не те кара да проверяваш подзадачи, размера им или кода;
...
└── phases/                     ← правилата на фазите, четат се по една, в момента на фазата
    ...
    ├── 4-plan.md               разделяне на подзадачи, нива
